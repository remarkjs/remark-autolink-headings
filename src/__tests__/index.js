...
test('should autolink headings', t => {
    ['append', 'prepend', 'wrap'].forEach(b => {
        let {contents} = remark().use(slug).use(html).use(headings, {behaviour: b}).processSync(base('input.md'));
...
test('should accept custom content', t => {
    const md = '# method';
    const {contents} = remark().use(slug).use(html).use(headings, {content: {type: 'text', value: '#'}}).processSync(md);
...
test('should do nothing if slugs are not used', t => {
    let {contents} = remark().use(headings).use(html).processSync(base('input.md'));
